...
// system swaps digits and separators, a reuse system swaps digits only, the nil path is unchanged, and a
// locale's own default resolves to the baked format.
...
struct NumberingSystemResolutionTests {
    ...
    static func rendered(_ minorUnits: Int64, locale: LocaleIdentifier, system: NumberingSystem?) -> String? {
    ...
    @Test("nil renders exactly as the plain entry point does")
    func nilMatchesDefault() {
        #expect(Self.rendered(1_234_56, locale: "en-GB", system: nil) == "£1,234.56")
    }

    @Test("An imposing system swaps digits and separators")
    func imposingSwapsBoth() {
        // arab: Arabic-Indic digits, ٫ decimal, ٬ grouping.
        #expect(Self.rendered(1_234_56, locale: "en-GB", system: .arabicIndic) == "£١٬٢٣٤٫٥٦")
    }
    ...
    func reuseSwapsDigitsOnly() {
        ...
        #expect(Self.rendered(1_234_56, locale: "en-GB", system: .nko) == "£߁,߂߃߄.߅߆")
    }

    @Test("Devanagari swaps digits, keeping Latin separators")
    func devanagariReuse() {
        #expect(Self.rendered(1_234_56, locale: "en-GB", system: .devanagari) == "£१,२३४.५६")
    }
    ...
    func ownDefaultIsBaked() throws {
        ...
        let requested = try #require(
            MoneyLocalization.moneyFormat(for: Self.currency("GBP"), locale: "bn", numberingSystem: .bengali)
        )
    ...
    func uncoveredLocaleNil() {
        #expect(
            MoneyLocalization.moneyFormat(for: Self.currency("GBP"), locale: "zz", numberingSystem: .arabicIndic) == nil
        )
    }
}
