...
struct GeneratedPluralRulesTests {
    ...
    func everyLanguageIsCovered(_ language: String) throws {
        let rules = try #require(MoneyLocalization.pluralRules[language])
    ...
    func regionsAreNotListed() {
        #expect(MoneyLocalization.pluralRules["en-GB"] == nil)
    }
    ...
    func oneWholeUnitIsSingular(_ language: String) throws {
        let one = try #require(MoneyLocalization.pluralRules[language]?[.one])
    ...
    func frenchHasItsOwnRules() throws {
        let french = try #require(MoneyLocalization.pluralRules["fr"])
