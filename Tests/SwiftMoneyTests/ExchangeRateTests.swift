...
struct ExchangeRateTests {
    ...
    func applyingMargin() throws {
        let mid = try #require(ExchangeRate<Currencies.EUR, Currencies.GBP>(quoting: 100, per: 100))   // 1.0
    ...
    func zeroMarginIsIdentity() throws {
        let mid = try #require(ExchangeRate<Currencies.EUR, Currencies.GBP>(quoting: 87, per: 100))
