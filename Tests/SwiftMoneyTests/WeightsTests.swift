...
struct WeightsTests {
    ...
    func literalEqualsValidatedCounterpart() {
        #expect(Weights([60, 30, 10]) == [60, 30, 10])
    }

    @Test("Equal weights are equatable")
    func equalWeightsAreEquatable() throws {
        let a = try #require(Weights([60, 30, 10]))
        let b = try #require(Weights([60, 30, 10]))
    ...
    func differentlyOrderedWeightsAreNotEqual() throws {
        let descending = try #require(Weights([60, 30, 10]))
        let ascending = try #require(Weights([10, 30, 60]))
