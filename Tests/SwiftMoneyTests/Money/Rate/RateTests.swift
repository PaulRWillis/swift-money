...
struct RateTests {
    ...
    func initElevenHundredths () throws {
        let r = try #require(Rate(numerator: 11, denominator: 100))
        #expect(r.numeratorValue == 11)
        #expect(r.denominatorValue == 100)
    }

    @Test("init reduces 22/200 to 11/100")
    func initReducesFraction () throws {
        let r = try #require(Rate(numerator: 22, denominator: 200))
        #expect(r.numeratorValue == 11)
        #expect(r.denominatorValue == 100)
    }

    @Test("init reduces 3/6 to 1/2")
    func initReducesHalf () throws {
        let r = try #require(Rate(numerator: 3, denominator: 6))
        #expect(r.numeratorValue == 1)
        #expect(r.denominatorValue == 2)
    }

    @Test("init with numerator == denominator reduces to 1/1")
    func initUnitRate () throws {
        let r = try #require(Rate(numerator: 100, denominator: 100))
        #expect(r.numeratorValue == 1)
        #expect(r.denominatorValue == 1)
    }

    @Test("init stores negative numerator")
    func initNegativeNumerator () throws {
        let r = try #require(Rate(numerator: -1, denominator: 10))
        #expect(r.numeratorValue == -1)
        #expect(r.denominatorValue == 10)
    }

    @Test("init reduces negative numerator (-22/200 → -11/100)")
    func initReducesNegativeFraction () throws {
        let r = try #require(Rate(numerator: -22, denominator: 200))
        #expect(r.numeratorValue == -11)
        #expect(r.denominatorValue == 100)
    }
    ...
    func initZeroNumerator () throws {
        let r = try #require(Rate(numerator: 0, denominator: 100))
        #expect(r.numeratorValue == 0)
        #expect(r.denominatorValue == 1)
    }

    @Test("init accepts Int64.max numerator")
    func initInt64MaxNumerator () throws {
        let r = try #require(Rate(numerator: .max, denominator: 1))
        #expect(r.numeratorValue == .max)
        #expect(r.denominatorValue == 1)
    }

    @Test("init returns nil for denominator == 0")
    func initZeroDenominatorIsNil () throws {
        #expect(Rate(numerator: 1, denominator: 0) == nil)
    }

    @Test("init returns nil for denominator < 0")
    func initNegativeDenominatorIsNil () throws {
        #expect(Rate(numerator: 1, denominator: -1) == nil)
    }

    @Test("init returns nil for numerator == Int64.min")
    func initInt64MinNumeratorIsNil () throws {
    ...
    func initFromDecimalElevenHundredths () throws {
        let decimal = try #require(Decimal(string: "0.11"))
        let r = try #require(Rate(decimal))
        #expect(r.numeratorValue == 11)
        #expect(r.denominatorValue == 100)
    }

    @Test("init from Decimal 0.25 produces 1/4")
    func initFromDecimalQuarter () throws {
        let decimal = try #require(Decimal(string: "0.25"))
        let r = try #require(Rate(decimal))
        #expect(r.numeratorValue == 1)
    ...
    func initFromDecimalOneAndHalf () throws {
        let decimal = try #require(Decimal(string: "1.5"))
        let r = try #require(Rate(decimal))
    ...
    func initFromDecimalOne () throws {
    ...
    func initFromDecimalTwo () throws {
        let decimal = try #require(Decimal(string: "2.0"))
        let r = try #require(Rate(decimal))
    ...
    func initFromDecimalZero () throws {
        let r = try #require(Rate(Decimal(0)))
        #expect(r.numeratorValue == 0)
        #expect(r.denominatorValue == 1)
    }

    @Test("init from Decimal -0.11 produces -11/100")
    func initFromDecimalNegative () throws {
        let decimal = try #require(Decimal(string: "-0.11"))
        let r = try #require(Rate(decimal))
        #expect(r.numeratorValue == -11)
        #expect(r.denominatorValue == 100)
    ...
    func initFromDecimalMatchesIntegerPair() throws {
        let decimal     = try #require(Decimal(string: "0.11"))
        let fromDecimal = try #require(Rate(decimal))
        let fromPair    = try #require(Rate(numerator: 11, denominator: 100))
        #expect(fromDecimal == fromPair)
    }

    @Test("init from Decimal preserves more than 10 decimal places (lossless)")
    func initFromDecimalHighPrecision () throws {
        // 0.12345678901234 = 12345678901234 / 10^14
        // GCD(12345678901234, 100_000_000_000_000) = 2
        // reduces to 6172839450617 / 50_000_000_000_000
        let decimal = try #require(Decimal(string: "0.12345678901234"))
        let r = try #require(Rate(decimal))
    ...
    func initFromDecimalElevenPlaces () throws {
        let decimal = try #require(Decimal(string: "0.00000000001"))
        let r = try #require(Rate(decimal))
        #expect(r.numeratorValue == 1)
    ...
    func initFromDecimalNaNIsNil () throws {
    ...
    func equalFractionsAreEqual () throws {
        let a = try #require(Rate(numerator: 11, denominator: 100))
        let b = try #require(Rate(numerator: 11, denominator: 100))
        #expect(a == b)
    }

    @Test("22/200 equals 11/100 after reduction")
    func reducedFractionsAreEqual () throws {
        let a = try #require(Rate(numerator: 22, denominator: 200))
        let b = try #require(Rate(numerator: 11, denominator: 100))
        #expect(a == b)
    }

    @Test("Different fractions are not equal")
    func differentFractionsAreNotEqual () throws {
        let a = try #require(Rate(numerator: 1, denominator: 10))
        let b = try #require(Rate(numerator: 1, denominator: 100))
        #expect(a != b)
    }
    ...
    func equalRatesSameHash () throws {
        let a = try #require(Rate(numerator: 22, denominator: 200))
        let b = try #require(Rate(numerator: 11, denominator: 100))
    ...
    func usableInSet () throws {
        let set: Set<Rate> = [
            try #require(Rate(numerator: 11, denominator: 100)),
            try #require(Rate(numerator: 22, denominator: 200)),
            try #require(Rate(numerator: 1,  denominator: 4)),
        ]
        #expect(set.count == 2)
    }
    ...
    func usableAsDictionaryKey () throws {
        ...
        dict[try #require(Rate(numerator: 11, denominator: 100))] = "eleven percent"
        let lookupKey = try #require(Rate(numerator: 22, denominator: 200))
    ...
    func compareSmaller () throws {
        let tenth = try #require(Rate(numerator: 1, denominator: 10))
        let fifth  = try #require(Rate(numerator: 1, denominator: 5))
        #expect(tenth < fifth)
        #expect(fifth > tenth)
        #expect(!(tenth < tenth))
    }
    ...
    func negativeIsLessThanZero () throws {
        let negative = try #require(Rate(numerator: -1, denominator: 10))
        let zero     = try #require(Rate(numerator: 0, denominator: 1))
    ...
    func sortable () throws {
        let unsorted: [Rate] = [
            try #require(Rate(numerator: 1, denominator: 2)),
            try #require(Rate(numerator: 1, denominator: 10)),
            try #require(Rate(numerator: 1, denominator: 4)),
        ]
    ...
    @Test("description formats as numerator/denominator",
          arguments: zip(
              [
                  try #require(Rate(numerator: 11, denominator: 100)),
                  try #require(Rate(numerator: -1, denominator: 10)),
                  try #require(Rate(numerator: 1,  denominator: 1)),
                  try #require(Rate(numerator: 0,  denominator: 1)),
              ] as [Rate],
    ...
    func decodesFromJson() throws {
        ...
        let expected = try #require(Rate(numerator: 11, denominator: 100))
        #expect(r == expected)
    }
