...
struct RateFormatStyleTests {
    ...
    func formatThreeQuarters() throws {
        let rate = try #require(Rate(numerator: 3, denominator: 4))
        #expect(rate.formatted(.fraction) == "3/4")
    }

    @Test("Formats -1/10 as fraction")
    func formatNegativeRate() throws {
        let rate = try #require(Rate(numerator: -1, denominator: 10))
    ...
    func formatGCDReduced() throws {
        let rate = try #require(Rate(numerator: 2, denominator: 4))
    ...
    func defaultModeIsFraction() throws {
        let style = Rate.FormatStyle()
        let rate = try #require(Rate(numerator: 7, denominator: 8))
    ...
    func formatNegativeDecimal() throws {
        let rate = try #require(Rate(numerator: -1, denominator: 4))
    ...
    func modeModifier() throws {
        let rate = try #require(Rate(numerator: 3, denominator: 4))
        let style = Rate.FormatStyle(.fraction, locale: enUS).mode(.decimal)
