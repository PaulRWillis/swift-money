...
struct Money_RateMultiplicationTests {
    ...
    func fractionalRateOperator() {
        ...
        let rate  = Rate(numerator: 1, denominator: 100)!
    ...
    func decimalOperatorMatchesRate() {
        let money = Money<TST_100>(minorUnits: 101)
        let viaDecimal = (money * Decimal(string: "0.01")!)!
        let viaRate    = money * Rate(numerator: 1, denominator: 100)!
    ...
    func nanTraps() async {
        await #expect(processExitsWith: .failure) {
            _ = Money<TST_100>.nan.multiplied(by: Rate(numerator: 1, denominator: 100)!)
        }
    }
    ...
    func nanOperatorTraps() async {
        await #expect(processExitsWith: .failure) {
            _ = Money<TST_100>.nan * Rate(numerator: 1, denominator: 100)!
        }
    }
}
