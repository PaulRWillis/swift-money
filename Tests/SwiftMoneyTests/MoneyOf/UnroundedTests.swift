...
struct UnroundedTests {
    ...
    func ratioScales() {
        #expect(roundsIdentically(GBP(minorUnits: 9_99).unrounded * "1/3") == GBP(minorUnits: 3_33))
    }
    ...
    func scalingInPlace() {
        ...
        #expect(byRatio == GBP(minorUnits: 9_99).unrounded * "1/3")
    ...
    func aChainKeepsAFraction() {
        let third = GBP(minorUnits: 10_00).unrounded * "1/3"
    ...
    func aProductThatCancelsDoesNotOverflow() throws {
        let threeOverTheLargest = try #require(Ratio(exactly: 3, over: .max))

        #expect(roundsIdentically(GBP.max.unrounded * threeOverTheLargest) == GBP(minorUnits: 3))
    }

    @Test("Scaling traps on overflow")
    func scalingTrapsOnOverflow() async {
        await #expect(processExitsWith: .failure) {
            blackHole(GBP.max.unrounded * "3/1")
        }
    }
    ...
    func scalingTrapsOnUnderflow() async {
        await #expect(processExitsWith: .failure) {
            blackHole(GBP.min.unrounded * "3/1")
        }
    }
    ...
    func settlingAFractionOfAnExtremeSteps() {
        let largest = GBP.max.unrounded * "1/3"
        let smallest = GBP.min.unrounded * "1/3"
    ...
    func differentDenominatorsAdd() {
        let aThird = GBP(minorUnits: 3_00).unrounded * "1/3"
    ...
    func amountsSubtract() {
        let aThird = GBP(minorUnits: 10_00).unrounded * "1/3"
        let twoThirds = GBP(minorUnits: 20_00).unrounded * "1/3"
    ...
    func sameLargeDenominatorAdds() throws {
        let oneQuintillionth = try #require(Ratio(exactly: 1, over: 1_000_000_000_000_000_000))
        let tiny = GBP(minorUnits: 1).unrounded * oneQuintillionth
    ...
    func settledMoneyJoinsInPlace() {
        var running = GBP(minorUnits: 9_99).unrounded * "1/3"
    ...
    func equalChainsAreEqual() {
        #expect(GBP(minorUnits: 10_00).unrounded * "2/6" == GBP(minorUnits: 10_00).unrounded * "1/3")
    }

    @Test("Equal chains hash alike")
    func equalChainsHashAlike() {
        let chains = Set([GBP(minorUnits: 10_00).unrounded * "2/6", GBP(minorUnits: 10_00).unrounded * "1/3"])
